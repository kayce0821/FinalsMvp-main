...
if ($row = $result->fetch_assoc()) {
    ...
    // Failsafe if user is somehow deleted while logged in
    $db_username = "Unknown";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update_profile'])) {
        ...
        // Since your 'user' table only has full_name, username, password, role, status
        // We will only update the full_name here.
        if (!empty($new_fullname)) {
            ...
            if ($update_stmt->execute()) {
                ...
                $db_fullname = htmlspecialchars($new_fullname); // Update UI immediately
            } else {
    ...
    elseif (isset($_POST['change_password'])) {
        ...
        if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            // STEP A: Fetch the user's current hashed password from the `user` table
            ...
            if ($row = $result->fetch_assoc()) {
                ...
                // STEP B: Verify if the typed current password matches the hash
                if (password_verify($current_password, $db_hashed_password)) {
                    
                    // STEP C: It matches! Hash the new password
...
<head>
    ...
    <style>
        ...
        .form-control { background-color: #fff; border: 1px solid #ced4da; border-radius: 0.375rem; padding: 0.6rem 1rem; color: #495057; }
                ...
                <div class="container-fluid">
                    <ul class="navbar-nav align-items-center">
                        ...
                        <li class="nav-item d-none d-sm-inline-block ms-2">
                            <a href="adminDashboard.php" class="nav-link font-weight-bold text-light p-0 text-decoration-none" style="font-size: 1.1rem; letter-spacing: 0.5px;">
                                Account Settings
                            </a>
                        </li>
                    </ul>
                                    ...
                                    <form method="POST" action="">
                                        ...
                                        <div class="row mb-3 mt-4">
                                            <div class="col-md-8">
                                                ...
                                                <div class="form-text text-muted small mt-1">Your password must be at least 8 characters long.</div>
                                            </div>
                                        </div>
